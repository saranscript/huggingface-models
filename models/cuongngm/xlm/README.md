...
def infer(image, processor, model, label2color):
    ...
    return image
processor = LayoutXLMProcessor.from_pretrained('cuongngm/xlm')
model = LayoutLMv2ForTokenClassification.from_pretrained("cuongngm/xlm")
# imgs = [img_path for img_path in Path('/your/path/imgs/').glob('*.jpg')]
img = Image.open(your image path)
outputs = infer(img_path, processor, model, label2color)
